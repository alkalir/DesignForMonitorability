...
This parameter can be set with one or more values from the list below, as they are not mutually exclusive.
...
- **Security:** monitoring systems for security support on checking that the system performs operations that it was intended to do. This can be done by checking the system state against a model, verifying some predefined signatures, or using filtering structures that allow only some processes to work.
